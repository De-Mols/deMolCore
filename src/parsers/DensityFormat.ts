...
export function DensityFormat(str: string, options: ParserConfig = {}) {
  const atoms: any[][] & Record<string, any> = [[]];
  const lattice: Record<string, number | Float32Array> = {};
  const assignbonds =
  ...
  const atomSpeciesNumber = new Int16Array(
    lines[6].trim().split(/\s+/) as any
  );
  let vaspMode = lines[7].trim();
