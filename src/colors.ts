...
export class CC {
  ...
  static color(hex?: unknown): unknown {
    ...
    const hexval = CC.getHex(hex as (string | number));
    const c = new Color(hexval);
    CC.cache[hexval] = c;
    return c;
  }
  ...
  static getHex(hex: unknown): unknown {
    if (Array.isArray(hex)) return hex.map(CC.getHex);
    if (typeof hex === "string") {
      ...
      return (window as any)?.$DeMol?.htmlColors[hex.toLowerCase()] || 0x000000;
    }
