...
export class Material extends EventDispatcher {
  ...
  setValues(
    ...
    for (const key in values) {
      const newValue: Material[keyof Material] = values[key as keyof Material];
      if (newValue === undefined) {
        console.warn("$DeMol.Material: '" + key + "' parameter is undefined.");
        continue;
      }
      if (key in this) {
        const currentValue = this[key as keyof Material];
